...
__declspec(dllexport) struct LZXstate *LZXinit(int window);

/* destroy an lzx state object */
__declspec(dllexport) void LZXteardown(struct LZXstate *pState);

/* reset an lzx stream */
__declspec(dllexport) int LZXreset(struct LZXstate *pState);

/* decompress an LZX compressed block */
__declspec(dllexport) int LZXdecompress(struct LZXstate *pState,
                  unsigned char *inpos,
